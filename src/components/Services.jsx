    ...
    <section style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(28px,4vw,44px) 20px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(230px,1fr))', gap: 16 }}>
        {services.map((s, i) => {
          ...
          return (
            <div key={s.id || s.title} style={card} data-reveal data-reveal-delay={i * 70}>
              <div style={{
              ...
              <h3 style={{ margin: '18px 0 6px', fontSize: 16.5, fontWeight: 650, letterSpacing: '-0.01em' }}>{s.title}</h3>
              <p style={{ margin: 0, fontSize: 14, lineHeight: 1.6, color: c.ink55 }}>{s.body}</p>
            </div>
          );
        })}
      </div>
